...
class RandomContrast:
    def __init__(self, min_c: float = 1, max_c: float = 1, clamp: tuple[float] | None = None) -> None:
        self.min_c = min_c
        self.max_c = max_c
        self.clamp = clamp
...
class RandomBrightness:
    ...
    def __call__(self, scene: torch.Tensor) -> torch.Tensor:
        ...
        if self.clamp is not None:
            adjusted = torch.clamp(scene, min=self.clamp[0], max=self.clamp[0])
...
class ProductDropout:
    def __init__(self, group: list[int], drop_rate: float, training: bool) -> None:
        self.group = group
...
@PRETRAIN_TRANSFORMS.register()
class RandomFlip:
    def __init__(self, flip_ud: bool, flip_lr: bool) -> None:
...
@PRETRAIN_TRANSFORMS.register()
def RGBColorJitter(brightness: float = 0.0, contrast: float = 0.0, saturation: float = 0.0, hue: float = 0.0):
...
@PRETRAIN_TRANSFORMS.register()
def GaussianBlur(kernel_size: int | Sequence, sigma: tuple[float] = [0.1, 2.0]):
...
def RandomResizedCrop(
    size: int | Sequence, scale: tuple[float] = (0.08, 1.0), ratio: tuple[float] = (3.0 / 4.0, 4.0 / 3.0)
):
    return transforms.RandomResizedCrop(size=size, scale=scale, ratio=ratio)


@PRETRAIN_TRANSFORMS.register()
def Normalize(mean: Sequence, std: Sequence, inplace: bool = False):
    return transforms.Normalize(mean=mean, std=std, inplace=inplace)


@PRETRAIN_TRANSFORMS.register()
class RGBGaussianHighPass:
    def __init__(self, sigma: int = 5, kernel: tuple[int, int] = (9, 9)) -> None:
        self.sigma = sigma
        self.kernel = kernel
...
class ImagePatchMask:
    ...
    def __call__(self, scene: torch.Tensor) -> torch.Tensor:
        ...
        patches = (
            grid_mask.unfold(2, self.patch_size, self.patch_size)
            .unfold(num_channel, self.patch_size, self.patch_size)
            .permute(0, 2, 3, 1, 4, 5)
            .reshape(batch_size, self.num_patch, 3, self.patch_size, self.patch_size)
        )
        ...
        grid_mask = torch.nn.functional.fold(
            patches_reorg, self.img_size, kernel_size=self.patch_size, stride=self.patch_size
        )
        grid_mask = grid_mask == 1

        if self.gaussian_mask:
            masked_image = torch.where(
                grid_mask[0], scene, torch.randn_like(scene, device=scene.device, dtype=scene.dtype) * 0.1 + 0.5
            )
        else:
            masked_image = torch.where(
                grid_mask[0], scene, torch.zeros_like(scene, device=scene.device, dtype=scene.dtype) + 0.5
            )
