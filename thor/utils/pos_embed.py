...
def get_2d_sincos_pos_embed_with_resolution(embed_dim, grid_size, res, cls_token=False, centered=False, device="cpu"):
    """
    grid_size: int of the grid height and width
    ...
    if cls_token:
        pos_embed = torch.cat(
            [
                torch.zeros([n, 1, embed_dim], dtype=torch.double, device=pos_embed.device),
                pos_embed,
            ],
            dim=1,
        )
...
# --------------------------------------------------------
def interpolate_pos_embed_thor(model, checkpoint_model, pos_embed_key):
    if pos_embed_key in checkpoint_model:
        pos_embed_checkpoint = checkpoint_model[pos_embed_key]
        embedding_size = pos_embed_checkpoint.shape[-1]
        # num_patches = model.patch_embed.num_patches
        num_patches = model.state_dict()[pos_embed_key].shape[-2]
        num_extra_tokens = 0  # model.pos_embed.shape[-2] - num_patches
        ...
        if orig_size != new_size:
            ...
            pos_tokens = pos_tokens.permute(1, 2, 0).flatten(0, 1)  # .squeeze(0)
