...
class CallWrapper:
    ...
    def __call__(self, *args):
        return self.func(*args, idx=self.idx)


class ControlledConsistentRandomResizedCrop(T.RandomResizedCrop):
    def __init__(
        self,
        sizes,
        scale=(0.2, 1.0),
    ...
    def generate_crop_params(self):
        img = torch.rand(1, self.sizes[0][0], self.sizes[0][1])
        self.ijhw = self.get_params(img, self.scale, self.ratio)
    ...
    def forward(self, img, idx=None):
        ...
        """
        if idx is not None:  # Override the count
            self.count = idx
        ...
        h, w = self.ijhw[2] * ratio_h, self.ijhw[3] * ratio_w

        return F.resized_crop(img, i, j, h, w, self.sizes[self.count], self.interpolation, antialias=self.antialias)


class ControlledConsistentRandomCrop(T.RandomCrop):
    ...
    def generate_crop_params(self):
        img = torch.rand(1, self.sizes[0][0], self.sizes[0][1])
        self.ijhw = self.get_params(img, self.sizes[0])
...
def get_satmae_transforms(
    ...
    if split == "train":
        ...
        img_transforms.append(T.Resize(size, interpolation=T.InterpolationMode.BICUBIC))
