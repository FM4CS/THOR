...
class NumpyEncoder(json.JSONEncoder):
    ...
    def default(self, obj):
        if isinstance(obj, np.ndarray) or isinstance(obj, np.floating) or isinstance(obj, np.number):
            return obj.tolist()
        return json.JSONEncoder.default(self, obj)


def read_json(path: str) -> dict[str, Any] | list:
    with open(path) as f:
        o = json.load(f)
    return o


def write_json(o: dict[str, Any] | list, path: str) -> None:
