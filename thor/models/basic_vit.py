...
from thor.utils.pos_embed import get_2d_sincos_pos_embed
...
class BasicVitLargeMultispectral(timm.models.vision_transformer.VisionTransformer):
    ...
    def __init__(self, global_pool=False, channel_groups=None, **kwargs):
        ...
        self.patch_embed = nn.ModuleList(
            [
                PatchEmbed(kwargs["img_size"], kwargs["patch_size"], len(group), kwargs["embed_dim"])
...
class BasicVitLargeRGB(timm.models.vision_transformer.VisionTransformer):
    ...
    def __init__(self, global_pool=False, channel_groups=None, **kwargs):
        ...
        self.pos_embed = nn.Parameter(torch.randn(1, num_patches + 1, kwargs["embed_dim"]) * 0.02, requires_grad=False)
        pos_embed = get_2d_sincos_pos_embed(self.pos_embed.shape[-1], int(num_patches**0.5), cls_token=True)
        self.pos_embed.data.copy_(torch.from_numpy(pos_embed).float().unsqueeze(0))
