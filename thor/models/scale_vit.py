...
class ScaleVitEncoder(timm.models.vision_transformer.VisionTransformer):
        ...
        **kwargs,
        ...
        for i, _group in enumerate(self.channel_groups):
            ...
            curr_pos_embed = get_2d_sincos_pos_embed_with_resolution(
                embed_dim, int(num_patches**0.5), input_res, cls_token=True
            )
    ...
    def forward(self, imgs, input_res=None):
        ...
        x = self.forward_features(x, input_res=None)
        return x
...
@MODELS.register()
def scale_vit_large_encoder(input_parmas, **kwargs):
    channel_groups = input_parmas.get(
        "groups",
        [
            ["S2:Red", "S2:Green", "S2:Blue", "S2:NIR"],
            ["S2:RE1", "S2:RE2", "S2:RE3", "S2:RE4"],
            ["S2:SWIR1", "S2:SWIR2"],
        ],
    )
    channel_group_gsds = input_parmas.get("group_gsds", [10, 20, 20])
    model_dict = dict(
        ...
        channel_group_gsds=channel_group_gsds,
        embed_dim=1024,
        depth=24,
        num_heads=16,
        mlp_ratio=4,
    ...
    return ScaleVitEncoder(**model_dict)


@MODELS.register()
def scale_vit_large_rgb_encoder(input_parmas, **kwargs):
    ...
    model_dict = dict(
        ...
        num_classes=0,  # don't need classification head
        channel_groups=channel_groups,
        input_res=input_parmas.get("input_res", 1),
        embed_dim=1024,
        depth=24,
        num_heads=16,
        mlp_ratio=4,
    ...
    return ScaleVitEncoderRGB(**model_dict)
